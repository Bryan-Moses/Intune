<#
    .SYNOPSIS
        Detection script that checks whether local MCP servers and desktop extensions are disabled.

    .DESCRIPTION
        Confirms Claude Desktop is installed, then checks HKLM:\SOFTWARE\Policies\Claude for
        isLocalDevMcpEnabled and isDesktopExtensionEnabled. Returns non-compliant if either value is
        missing, incorrect, or the policy path does not exist. Pairs with the matching remediation
        script.
...
#### Logging Variables ####
$Script:ScriptName = "Detect-DisableDesktopExtensionsAndManualMCPConfigs"
...
# Required policy state (0 = disabled)
$RequiredPolicies = @{
...
if ($NonCompliant) {
    ...
    Write-Output "COMPLIANT: Local MCP and desktop extensions are disabled."
    Stop-Transcript
    Exit 0
}
