<#
    .SYNOPSIS
        Remediation script that enables Claude Cowork and Claude Code.

    .DESCRIPTION
        Creates HKLM:\SOFTWARE\Policies\Claude if needed and sets secureVmFeaturesEnabled and
        isClaudeCodeForDesktopEnabled to 1 (DWORD), enabling the Cowork and Claude Code features in
...
#### Logging Variables ####
$Script:ScriptName = "Remediate-EnableClaudeCodeAndCowork"
...
try {
    ...
    if ($Errors.Count -gt 0) {
        ...
        Write-Output "REMEDIATED: Cowork and Claude Code enabled."
        Stop-Transcript
        Exit 0
    }
}
