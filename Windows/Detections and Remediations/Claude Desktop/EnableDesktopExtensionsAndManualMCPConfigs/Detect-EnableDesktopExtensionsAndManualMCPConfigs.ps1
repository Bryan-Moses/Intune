<#
    .SYNOPSIS
        Detection script that checks whether local MCP servers and desktop extensions are enabled.
    ...
    .DESCRIPTION
        ...
        missing, set to 0, or the policy path does not exist. Pairs with the matching remediation
        script.
...
#### Logging Variables ####
$Script:ScriptName = "Detect-EnableDesktopExtensionsAndManualMCPConfigs"
...
if ($NonCompliant) {
    ...
    Write-Output "COMPLIANT: Local MCP and desktop extensions are enabled."
    Stop-Transcript
    Exit 0
}
